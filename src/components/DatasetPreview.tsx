      ...
      {pairs.map((p, i) => (
        ...
        >
          <summary className="cursor-pointer theme-text leading-relaxed flex items-start gap-4 list-none outline-none">
            ...
            <div className="flex-1 min-w-0 pt-1">
              ...
              <div className="flex items-center gap-2 mt-1 opacity-40">
                <span className="text-[8px] uppercase tracking-widest font-black font-mono">Sample UID: {Math.random().toString(36).substring(7).toUpperCase()}</span>
              </div>
            </div>
          ...
          <div className="mt-5 space-y-4 pl-12 animate-premium relative">
            ...
            {p.think && (
              <div className="bg-black/60 border border-white/5 rounded-xl p-4 font-mono text-xs-fluid theme-muted whitespace-pre-wrap shadow-inner relative group/think">
                <div className="absolute -left-8 top-1/2 -translate-y-1/2 w-6 h-0.5 bg-white/5" />
                <span className="text-[9px] uppercase tracking-[0.2em] theme-accent font-black mb-2 block opacity-60 group-hover/think:opacity-100 transition-opacity italic">Chain of Thought</span>
                {p.think}
              </div>
            )}
